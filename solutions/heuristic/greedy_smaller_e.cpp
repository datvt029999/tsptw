...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i <= n; i++)
    {
        ...
        {
            if (!visited[j] && time_finishing_delivering + t[previous_point][j] <= l[j] && (e[j] < best_deadline || (e[j] == best_deadline && t[previous_point][j] < best_distance)))
            {
                best_point = j;
                best_deadline = e[j];
        ...
        {
            feasible = false;
            cout << "The greedy heuristic based on smaller e[i] gives infeasible solution." << endl;
            break;
        }
